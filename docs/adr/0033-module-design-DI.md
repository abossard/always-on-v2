...
The domain module uses this interface but doesn't define it — the port lives in `Storage.cs` alongside its implementations, because the port's shape is driven by the adapters' capabilities (see [ADR-0034](0034-simplified-hexagonal-architecture-DI.md)).
...
- [ADR-0032: Coding Principles](0032-coding-principles-DI.md) — Grokking Simplicity's data/calculation/action separation
- [ADR-0034: Simplified Hexagonal Architecture](0034-simplified-hexagonal-architecture-DI.md) — How modules compose into the architecture
