...
- **Status**: Proposed
- **Date**: 2026-03-11
- **Relates to**: ADR-0001 (workload deployment), ADR-0025 (deployment strategy)
...
- **Progressive delivery** — Flagger + Istio Gateway API enables automated canary with metrics-based promotion, directly addressing ADR-0025 Option 5.
...
- **Operational complexity** — Multiple new components (Flux, Istio, Flagger, Fleet CRPs) increase the surface area for debugging. Mitigate with strong observability (ADR-0010) and runbooks.
