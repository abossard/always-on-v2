...
[ADR-0039](0039-orleans-ingress-DI.md) established that Front Door routes directly to an Internal Load Balancer backed by Orleans pods — eliminating the NGINX ingress tier. That ADR addressed a single logical application (one silo cluster, one origin group in Front Door).
...
Private Link to an Internal Load Balancer is only available in **Front Door Premium**. This is the same requirement established in [ADR-0039](0039-orleans-ingress-DI.md). All routing, WAF, and Private Link features are configured through Bicep — there is no manual portal configuration.
...
- **NGINX in-cluster for cross-domain routing** — Would allow path routing without Front Door Premium. Rejected: re-introduces the NGINX tier that [ADR-0039](0039-orleans-ingress-DI.md) eliminated; Front Door Premium is already required for Private Link.
...
- [ADR-0039: Orleans Ingress — Front Door Direct to Internal Load Balancer](0039-orleans-ingress-DI.md)
- [ADR-0019: Global Load Balancing — Azure Front Door](0019-global-load-balancing-UI.md)
- [ADR-0001: Compute Platform — AKS](0001-compute-platform-DI.md)
- [ADR-0002: Application Framework — Orleans](0002-application-framework-DI.md)
