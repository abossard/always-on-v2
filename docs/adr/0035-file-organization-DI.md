# ADR-0035: File Organization — Combine What Belongs Together
...
- **One file for everything** — Putting Domain + Endpoints + Storage in one file would be too large (~500 lines) and mix concerns (pure domain with I/O). The three-file split reflects the three architectural roles (see [ADR-0034](0034-simplified-hexagonal-architecture-DI.md)).
...
- [ADR-0034: Simplified Hexagonal Architecture](0034-simplified-hexagonal-architecture-DI.md) — The three-module structure
