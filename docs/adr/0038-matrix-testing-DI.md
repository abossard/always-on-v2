...
With a simplified hexagonal architecture ([ADR-0034](0034-simplified-hexagonal-architecture-DI.md)), we have multiple storage adapters behind the same port (InMemory, Cosmos DB). Every adapter must behave identically — if tests pass against InMemory but fail against Cosmos, we have a port contract violation.
...
- [ADR-0034: Simplified Hexagonal Architecture](0034-simplified-hexagonal-architecture-DI.md) — Port + multiple adapters
- [ADR-0032: Coding Principles](0032-coding-principles-DI.md) — Data-driven edge cases, no exceptions for expected failures
