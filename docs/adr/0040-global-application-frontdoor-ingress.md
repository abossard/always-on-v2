# ADR-0040: Global Application — Front Door as Multi-Silo Ingress
...
[ADR-0039](0039-orleans-ingress.md) established that Front Door routes directly to an Internal Load Balancer backed by Orleans pods — eliminating the NGINX ingress tier. That ADR addressed a single logical application (one silo cluster, one origin group in Front Door).
...
Private Link to an Internal Load Balancer is only available in **Front Door Premium**. This is the same requirement established in [ADR-0039](0039-orleans-ingress.md). All routing, WAF, and Private Link features are configured through Bicep — there is no manual portal configuration.
...
- **NGINX in-cluster for cross-domain routing** — Would allow path routing without Front Door Premium. Rejected: re-introduces the NGINX tier that [ADR-0039](0039-orleans-ingress.md) eliminated; Front Door Premium is already required for Private Link.
...
- **Negative**: Front Door Premium is more expensive than Standard. Justified by Private Link security posture and WAF; already accepted in ADR-0039.
...
- [ADR-0039: Orleans Ingress — Front Door Direct to Internal Load Balancer](0039-orleans-ingress.md)
- [ADR-0019: Global Load Balancing — Azure Front Door](0019-global-load-balancing.md)
- [ADR-0001: Compute Platform — AKS](0001-compute-platform.md)
- [ADR-0002: Application Framework — Orleans](0002-application-framework.md)
