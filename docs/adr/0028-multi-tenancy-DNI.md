...
- Zero-trust security (ADR-0015) demands strong data isolation between tenants.
- Operational complexity increases significantly with shared infrastructure.
- The current player progression API (ADR-0027) uses `playerId` as the partition key with no tenant dimension.
